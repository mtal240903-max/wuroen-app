...
export default function CreateCompanyScreen({ navigation }) {
  const { createNewCompany, currentWorkspace } = useWorkspace();
  
  const [name, setName] = useState('');
  const [description, setDescription] = useState('');
  const [type, setType] = useState('Exploitation / Ferme');
  ...
  const types = ['Exploitation / Ferme', 'Entreprise / Startup', 'Laboratoire / Centre'];
  const sectors = ['Production Animale', 'Élevage', 'Agri-Tech', 'R&D / Science', 'Tech / Dév'];
  ...
  const handleCreate = async () => {
    if (!currentWorkspace?._id) {
      Alert.alert('Erreur', 'Aucun espace de travail actif trouvé.');
      return;
    }
    ...
    try {
      ...
      await createNewCompany(currentWorkspace._id, formData);
      ...
      Alert.alert('Erreur', "La structure n'a pas pu être créée.");
    } finally {
    ...
    <View style={styles.container}>
      <View style={styles.navbar}>
        ...
        <View style={styles.navTitleContainer}>
          ...
          <Text style={styles.navSubtitle}>Ajouter une entité au workspace</Text>
        </View>
      </View>
